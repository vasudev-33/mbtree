...
DROP PROCEDURE IF EXISTS `getRightBoundaryKey` $$
CREATE DEFINER=`vasu`@`localhost` PROCEDURE `getRightBoundaryKey`(in threadId int, in levelId int,in leafId int, in key1 int)
BEGIN
